...
快手极速版
变量名：ksjsbcookie
格式：did=xxx;kuaishou.api_st=xxx;client_salt=xxx;
				...
				if(_0x2ce750.result==1){
					if(_0x906fe1==true){
						if((_0x2ce750.data.commonAwardPopup!=null)&&(_0x2ce750.data.commonAwardPopup!='')){
							...
							notify.sendNotify('快手极速版开宝箱通知', '账号' + $.index + '的' + $.nickname + '开宝箱获得' + _0x2ce750.data.commonAwardPopup.awardAmount + '金币');
						...
						if(_0x510ad7==0){
							...
							notify.sendNotify('快手开宝箱通知', '账号' + $.index + '的' + $.nickname + '今天开宝箱次数已用尽！');
						}else{
							console.log('账号  '+$.index+'  ['+$.nickname+']开宝箱冷却时间还有'+(_0x510ad7/1000)+'秒');
							notify.sendNotify('开宝箱通知', '账号' + $.index + '的' + $.nickname + '还需要等待' + (_0x510ad7/1000) + '秒才能继续开宝箱！');
						}
					}
				}else{
...
async function _0x449ff6(starttime,endtime,zero,type,timeout=3*1000){
	...
	if(type=='guangjie'){
		...
		if($.sig3==''){
			for(let i=0;i<1;i++){
				console.log('账号  '+$.index+'  ['+$.nickname+']Api[1]访问失败，开始重试'+(i+1)+'/1');
				await $.wait(2000);
				await getsig(_0x4e5eda,''+$.salt,'/rest/r/reward/task/getActivityReward');
				if($.sig3!=''){
					break;
			...
			if($.sig3==''){
				for(let i=0;i<1;i++){
					console.log('账号  '+$.index+'  ['+$.nickname+']Api[2]访问失败，开始重试'+(i+1)+'/1');
					await $.wait(2000);
					await getsig2(_0x4e5eda,''+$.salt,'/rest/r/reward/task/getActivityReward');
					if($.sig3!=''){
						break;
